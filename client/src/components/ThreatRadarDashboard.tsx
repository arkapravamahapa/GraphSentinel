...
export const ThreatRadarDashboard: React.FC<ThreatRadarDashboardProps> = ({
  ...
  // Navigation Destination on Left Rail
...
=======
  ...
  useEffect(() => {
    if (!selectedNode && !selectedThreat) {
      setSelectedNode(nodes[8]); // Default to Sybil threat node for rich initial context
    }
  }, []);
...
<<<<<<< HEAD
    ...
    ws.onmessage = (event) => {
      ...
      if (isAttack) {
        setLogs((prev) => [...prev.slice(-49), 
          ...
          { id: String(Date.now() + 2), timestamp: now, type: 'agent', message: 'CrewAI Agent: Autonomous threat threshold met. Ready for on-chain mitigation.' }
        ]);
  ...
  // TRIGGER SIMULATE ATTACK ENDPOINT
...
<<<<<<< HEAD
  ...
  // TRIGGER ON-CHAIN DEFENSE EXECUTION ENDPOINT
  const handleExecuteDefense = async () => {
    setIsExecutingDefense(true);
    setToastMessage('🛡️ Broadcasting DefensePool.sol interceptor to Base Sepolia...');

    try {
      const response = await fetch('http://127.0.0.1:8000/api/execute-defense', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          target_token: "0xEBFBA4aaF595a1aB3F22Bd411954bcDdf7CAeDe6",
          amount: 500
        })
      });
      ...
      if (data.status === 'success') {
        setToastMessage('✅ Defense executed successfully on-chain!');
        const now = new Date().toTimeString().split(' ')[0];
        setLogs((prev) => [...prev, { id: String(Date.now()), timestamp: now, type: 'defense', message: data.message }]);
      } else {
        setToastMessage(`❌ Execution failed: ${data.message}`);
      }
    } catch (error) {
      console.error("Failed to execute defense", error);
      setToastMessage('❌ Connection error executing on-chain defense.');
    }
...
<<<<<<< HEAD
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <button
            ...
            style={{
              background: isExecutingDefense ? '#334155' : 'linear-gradient(135deg, #059669, #047857)',
              color: '#ffffff',
              border: 'none',
              borderRadius: '7px',
              padding: '7px 12px',
              fontSize: '12px',
              fontWeight: 600,
              cursor: isExecutingDefense ? 'not-allowed' : 'pointer',
              boxShadow: '0 0 15px rgba(5, 150, 105, 0.3)',
            }}
          >
            <span>{isExecutingDefense ? '🛡️ Executing TX...' : '🛡️ Trigger Defense'}</span>
          </button>
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0, overflow: 'hidden' }}>
        ...
        >
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '8px', padding: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '12px', fontWeight: 600, color: '#a855f7' }}>Web3 Interceptor</span>
                  <span style={{ fontSize: '10px', background: '#6b21a8', color: '#f3e8ff', padding: '1px 5px', borderRadius: '4px' }}>Connected</span>
                </div>
                <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '4px' }}>
                  Hardhat DefensePool.sol signer
                </div>
              </div>
            </div>
          </div>
        ...
        >
          ...
          {/* Middle Row: Network Graph & Agent Terminal */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '16px', minHeight: '380px' }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#22d3ee' }} />
                  <h3 style={{ margin: 0, fontSize: '15px', fontWeight: 600, color: '#e2e8f0', textTransform: 'uppercase' }}>
...
<<<<<<< HEAD
          ...
          {/* Bottom Row: GAF Heatmap & Recent Threat Log */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '16px' }}>
            {/* GAF Heatmap Viewer */}
            <div
              style={{
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
                ...
                <span style={{ fontSize: '11px', color: attackActive ? '#f43f5e' : '#34d399', background: attackActive ? 'rgba(244, 63, 94, 0.1)' : 'rgba(52, 211, 153, 0.1)', padding: '2px 8px', borderRadius: '4px', fontFamily: 'monospace' }}>
                  LIVE ML INFERENCE
                </span>
              </div>
              ...
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                ...
                <div style={{ fontSize: '12px', color: '#94a3b8', lineHeight: 1.5 }}>
                  ...
                  <p style={{ margin: '0 0 8px 0' }}>
                    Converts price time-series into a 2D polar matrix via <code>pyts</code>. The red diagonal band indicates synchronized wash volume across Sybil clusters.
                  </p>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
                  ...
                  <tbody>
                    <tr style={{ borderBottom: '1px solid #1e293b' }}>
                      ...
                      <td style={{ padding: '8px', fontFamily: 'monospace', color: '#38bdf8' }}>0x2122...f0ad</td>
                    </tr>
                    <tr style={{ borderBottom: '1px solid #1e293b' }}>
